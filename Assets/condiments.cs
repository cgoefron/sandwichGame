...
public class condiments : MonoBehaviour{
	public static condiments Instance;

	/// <summary>
	/// A single paint decal to instantiate
	/// </summary>
	public Transform KetchupPrefab;
	...
	void Awake()
	{
		if (Instance != null) Debug.LogError("More than one Condiment has been instanciated in this scene!");
		Instance = this;

		if (KetchupPrefab == null) Debug.LogError("Missing Ketchup decal prefab!");
	}
	...
	void Update()
	{
		...
		if (Input.GetMouseButtonDown(0))
		{
			...
			if (Physics.Raycast(ray, out hit, Mathf.Infinity))
			{
				...
				Condiment(hit.point + hit.normal * (SplashRange / 4f));
			}
		}
	}

	public void Condiment(Vector3 location)
	{
		//DEBUG
		mHitPoint = location;
		mRaysDebug.Clear();
		...
		while (n <= drops)
		{
			...
			if (Physics.Raycast(location, fwd, out hit, SplashRange))
			{
				...
				var condimentSplatter = GameObject.Instantiate(KetchupPrefab,
					hit.point,
				...
				condimentSplatter.localScale = new Vector3(
					condimentSplatter.localScale.x * scaler,
					condimentSplatter.localScale.y * scaler,
					condimentSplatter.localScale.z
				);

				// Random rotation effect
				var rater = Random.Range(0, 359);
				condimentSplatter.transform.RotateAround(hit.point, hit.normal, rater);


				// TODO: What do we do here? We kill them after some sec?
				Destroy(condimentSplatter.gameObject, 25);
			}
